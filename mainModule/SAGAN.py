...
class DPBS:
    ...
    def modelTraining(self, resumeTraning=False, overFitTest=False, dataSamples = None):
        ...
        while currentStep < self.totalSteps:
            ...
            for LRImages, HRGTImages in trainingImageLoader:
                ...
                '''
                # Patch Extract
                patchDim = highResFake.shape[2]//2
                ranDim = random.randint(0, patchDim)
                highResFakePatch = highResFake[:,:, ranDim: ranDim + patchDim,ranDim: ranDim + patchDim]
                highResRealPatch = highResReal[:,:, ranDim: ranDim + patchDim,ranDim: ranDim + patchDim]
                print(highResFakePatch.shape, highResRealPatch.shape, targetReal.shape)'''
                ...
                generatorContentLoss =  reconstructionLoss(highResFake, highResReal) + colorLoss(highResFake, highResReal)#\
                ...
                if (currentStep  + 1) % self.interval/2 == 0:
                    ...
                    print(Fore.YELLOW + "Steps |",bar,Fore.YELLOW + "| LossEG: {:.4f}, LossED: {:.4f}, PSNR: {:.4f}".format(lossEG, lossED, psnr),end='\r')
                ...
                if (currentStep + 1) % 100 == 0:
                    ...
                    summaryInfo = { 
                                    ...
                                    'STAGAN Images' : self.unNorm(highResFake),
    ...
    def modelInference(self, testImagesPath = None, outputDir = None, resize = None, validation = None, noiseSet = None, steps = None):
        ...
        #modelStep = glob.glob(self.checkpointPath + "*/")
        #modelName = self.modelName

        #print (modelStep)
        #for m in modelStep:
        #    self.checkpointPath = m
        #    self.modelName = modelName + m.split("/")[-2]
        ...
        with torch.no_grad():
            for noise in noiseSet:
                ...
                for imgPath in testImageList:
                    ...
                    # Evaluation 
                    save_image(self.unNorm(output[0]), 'modelOutput.png')
                    gt = io.imread(imgPath)
                    target = io.imread('modelOutput.png')
                    psnrScore = psnr(gt, target)
                    ssimScore = ssim(gt, target, multichannel=True)
                    PSNRval.append(psnrScore)
                    SSIMVal.append(ssimScore)
                    if imageCounter % 2 == 0:
    ...
    def modelSummary(self,input_size = None):
        ...
        customPrint(Fore.YELLOW + "STAGAN (Generator)", textWidth=self.barLen)
        summary(self.attentionNet, input_size =input_size)
        print ("*" * self.barLen)
        print()

        customPrint(Fore.YELLOW + "STAGAN (Discriminator)", textWidth=self.barLen)
